...
async function content_generation(post) {
    ...
    for (let c = 0; c < company_count; c++) {
        ...
        if (search_text[c] != null) {
            ...
            if (search_count == 0) {
                if (search_text[c][0] === '/' && search_text[c] != 1) search_message = `商品番号「${Number(search_text[c].slice(1))}」で検索中：商品が見つかりませんでした`;
                else search_message = `「${search_text[c].trim()}」で検索中：商品が見つかりませんでした`;
            } else {
                if (search_text[c][0] === '/' && search_text[c] != 1) search_message = `商品番号「${Number(search_text[c].slice(1))}」を表示しています`;
                else search_message = `「${search_text[c].trim()}」で検索中：${search_count}件見つかりました`;
            }

            const searched_text = Object.assign(document.createElement('p'), {
                classList: 'searched_text',
                textContent: search_message
            });
...
const element_process = [
    {   // 詳細情報開く
        element_id: 'info_button',
        event: 'click',
        handler: () => {
            document.getElementById('info_modal_back').classList.add('show');
        }
    },
    {   // 詳細情報閉じる
        element_id: 'info_close_button',
        event: 'click',
        handler: () => document.getElementById('info_modal_back').classList.remove('show')
    },
    {   // 検索開く
